...
## Provenance Docstring (Every AI-Written Script)

Every source file you create opens with a docstring or header comment recording, in one or two sentences, the request that produced it. This is **required output**, exempt from "Simplicity First" and "Surgical Changes" below, and never stripped or flagged as unrequested scope by a review or simplification pass. It is a pack convention, not an external standard.

- **Where.** The top of the file, after only what the language requires first — shebang, encoding declaration, license/SPDX header — and never breaking a construct the language needs contiguous at the top.
- **What.** The language's own idiom: a module docstring where that is the convention, otherwise a comment block. Keep the file's normal summary line first if it has one, then, after a blank line, `Original request:` on its own line so it stays greppable. Paraphrase in the requester's terms, not the implementation's, and strip anything that would close the comment or docstring you are writing (`*/`, `"""`, `#>`, `-->`). Never paste a long prompt verbatim; redact any secret, credential, or personal data and say you redacted it rather than dropping it silently.
- **How it reads.** Past tense and descriptive — "asked for X", never "do X" — so tools that scan comments cannot mistake it for a live instruction. Never phrase it so a tool reads the file as machine-generated; in particular never include `Code generated`, `DO NOT EDIT`, `@generated`, or `autogenerated file`, which make linters and review tools silently skip the file.
- **Limits.** New files you author, and only those in a programming or scripting language: do not retrofit existing ones, and skip config, markup, docs and data (`*.json`, lockfiles, minified or compiled output) unless the file is itself an executable script. It records the originating request, not current behavior, and is never updated as the file evolves.
