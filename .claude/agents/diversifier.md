---
name: diversifier
description: "HarnessFlow worker subagent — spawn only from a HarnessFlow workflow step, never by autonomous delegation. Proposes five structurally different alternative plans to the current plan — including a risky, an aggressive, and a rare one — each with a calibrated probability that it beats the current plan."
