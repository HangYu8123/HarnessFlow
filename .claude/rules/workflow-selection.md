...
- Effort is the second dial on the same spawn, never optional: the `subagent_effort` header (`inherit` | `low` | `medium` | `high` | `xhigh` | `max`; templates ship `low`), replaced by `online_researcher_effort` (templates ship `medium`) for the Online Researcher only. `inherit` means follow the session and add nothing; any other level goes in the agent definition's `effort:` field where the spawn uses one, and otherwise into the prompt as `effort: <level> — binding budget, not a hint`, because the `Task` tool has no per-invocation effort parameter. In a `dispatch:` loop both levels cross the boundary unchanged.
