...
- Subagents must follow the shared Subagent Launch Contract and use the same model as the main agent by default — in `mode: fast` the default main model is Sonnet 4.6, so subagents default to Sonnet 4.6; a `subagent_model` header overrides this with a specific model.
