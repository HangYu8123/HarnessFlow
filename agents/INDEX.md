...
| `diversifier` | **Diversifier** — Alternative-plan generation from the goal, never the draft (3–5 constraint-fenced, invariant-preserving plans + calibrated `P(better)` + `graftable:` components + an `if you relax` tail) | Read, Grep, Glob | read-only | code, debug, refactor, exec, pr, correctness_check |
