---
name: 'Cmd/Skill Execution (Codex)'
description: 'Instructions for executing commands and skills with structured planning and validation — Codex CLI native'
---
...
When asked to execute cmds/skills, always, first read the following files .github/harness_coding_instructions/repo_info (REFER AS [key md files]):
...
2. the main agent creates two subagents and **[PARALLEL EXECUTION via Codex agent workers — launch in parallel; if parallel subagents are not supported in your CLI environment, run them sequentially — sequential execution produces equivalent results]** (**Focus Analyst** via `agents/focus-analyst.agent.md`; **Free Analyst** via `agents/free-analyst.agent.md`), pass [inputs] to the two subagents. The two subagents must be launched in parallel. The two subagents read through [key md files]. Then:
...
4. the main agent creates two subagents: **Devils Advocate** (`agents/devils-advocate.agent.md`) and **Online Researcher** (`agents/online-researcher.agent.md`) **[PARALLEL EXECUTION via Codex agent workers — launch in parallel; if parallel subagents are not supported in your CLI environment, run them sequentially — sequential execution produces equivalent results]**, pass [final plan] and the target cmds/skills from [inputs] to the subagents.
...
5. Then, the main agent must print the updated [final plan], so the user can review it. **If the user has requested for no execution, THE MAIN AGENT MUST STOP AFTER PRINTING THE FINAL PLAN. OTHERWISE, KEEP PROCEEDING TO STEP 6**. If the user has NOT SPECIFIED or requires execution, the main agent must continue to step 6.
...
6.5. Skip — Claude-native skills are not available in Codex. Instead, the main agent performs a manual review of execution output for any anomalies before proceeding.

7. the main agent creates two subagents and **[PARALLEL EXECUTION via Codex agent workers — launch in parallel; if parallel subagents are not supported in your CLI environment, run them sequentially — sequential execution produces equivalent results]** (**Senior Engineer** via `agents/senior-engineer.agent.md`; **QA Engineer** via `agents/qa-engineer.agent.md`). Then:
