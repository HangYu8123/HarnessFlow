...
description: 'Fast correctness review for Claude Code: main-agent scope-driven inspection with optional script runs and parallel challenge + research subagents. Read-only'
---
...
Read [key md files]. If important files or target functionalities are specified in [inputs], read them. Condense into a [repo context digest] and identify [important information] — the most relevant code, scripts, and functionalities. Decide the **scope**: whole-repo (include the full pipeline diagram) or target functionality (include upstream/downstream context).

### Step 2 - Correctness Analysis
Based on [repo context digest] + [important information] + the chosen scope, the main agent lists the relevant files, orders them by pipeline flow, reads them, and examines correctness:
