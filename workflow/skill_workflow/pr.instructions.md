...
1. **Review skills** (`true` = Claude Code native · `local` = vendored pack skills; see [`_lib/review_skills.md`](../../_lib/review_skills.md)): PR re-organization authors no new logic, so these run only when source files were actually edited (e.g., conflict resolution).
