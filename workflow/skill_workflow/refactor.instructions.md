...
2. **Native review skills (platform-conditional):**
   - **If the main agent is Claude Code (or another Claude agent with Claude Code skills available):** run the native review skills **once** via [`skills/claude-native-skills-subagents/SKILL.md`](../../skills/claude-native-skills-subagents/SKILL.md) — that skill is the only caller: it runs `/simplify` first on the resulting diff (recorded as [simplify]), then `/code-review` (recorded as [code-review]). Do not run `/simplify` or `/code-review` yourself in addition to the skill. If the native skills are unavailable, skip this sub-step.
