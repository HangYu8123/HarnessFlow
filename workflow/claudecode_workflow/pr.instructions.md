---
name: 'PR Creation (Claude Code)'
description: 'Instructions for breaking down and creating pull requests from feature branches — Claude Code CLI native'
---
...
7. the main agent creates an **Implementer** subagent (`agents/implementer.agent.md`), pass [final pr plan], [inputs], and [repo context digest] to the subagent. **Implementer Model Verification:** See `_lib/workflow_contract.md` §Implementer Model Verification Fallback (skip retry loop in Claude Code — model is inherited automatically). The subagent (or the main agent, if falling back) receives [repo context digest] and [breakdown-pr skill]. Then based on [final pr plan], the subagent executes the PR stack creation following the [breakdown-pr skill] step 6 (Execute Only After Approval):
