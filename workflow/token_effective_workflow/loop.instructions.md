...
2. **Act (delegated) — the main agent never edits/fixes/runs the body work itself.**
   ...
   - **Free-form:** spawn a **body-worker subagent** (model = `subagent_model`), passing the **[work order]** defined above (and nothing beyond it). It performs the action and returns the **[iteration report]** only. When the action is code or command work, the body-worker may be `agents/implementer.agent.md` or `agents/executor.agent.md`.
