...
**Role emulation** (see [`_lib/workflow_contract.md`](../../_lib/workflow_contract.md) §Main-Agent Role Emulation): the controller performs these roles itself — no extra subagent spawned. Read `agents/focus-analyst.agent.md`, `agents/broad-analyst.agent.md`, `agents/free-analyst.agent.md`, and `agents/senior-engineer.agent.md` (spec consolidation), and apply each as a lens on the draft [loop spec] — depth on the verifier and the files the body will touch, the pipeline upstream→downstream for what the metric could miss, a free-judgment pass, and a senior-engineer consolidation that rejects game-able metrics and unverifiable criteria — into the [loop spec] the subagents below validate.
