...
5. **Reflect & ledger.** Append a [loop ledger] entry for iteration N capturing: **action** (free-form, or dispatched family+mode); **code changes** (files touched + one-line diff summary, or "none"); **metric** (`before → after`, i.e. the improvement); **observation + blocker**; **noteworthy** (decisions, surprises, regressions, lessons); **exit-check result** (incl. the exit-gater's verdict when the gate was checked). Carry one short lesson forward to the next iteration.
