...
For iteration N = 1, 2, …:
   ...
   b. **Act (delegated) — the controller never edits/fixes/runs the body itself.**
      ...
      - **Dispatch:** spawn one depth-1 **sub-main agent** (model = `dispatch_main_model`) to run `workflow/<mode dir>/<family>.instructions.md` (`<mode dir>`: `general` → `general_workflow`, `fast` → `token_effective_workflow`, `skill` → `skill_workflow`) **as that family's main agent** (default the dispatched family to `mode: general` for this general loop unless [input 3] says otherwise), spawning that family's own subagents at the next level with model = `dispatch_subagent_model`, and to return only a **compact iteration summary**. **Platform-conditional:** on Claude Code nested subagents are supported; on Codex / VS Code Copilot (limited nesting) the controller runs the family inline — sequential, equivalent results.
