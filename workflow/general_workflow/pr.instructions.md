...
The main agent incorporates [valid criticisms] and [online resource], and updates [final pr plan] accordingly. When [diverse plans] was produced, it dispositions every alternative against its own [final pr plan] per [`_lib/workflow_contract.md`](../../_lib/workflow_contract.md) §Diversifier Contract → Pick — `adopt` · `adopt-part <what>` · `same-as-draft` · `park` · `reject <reason>`, one line each — restating [final pr plan] on an adopted alternative, merging adopt-part components, and writing parked ones to `known_issues.md` §Untaken options.
