...
3. **Skip skills that do not apply at planning time** — e.g. post-implementation/review-only skills such as `claude-native-skills-subagents`, `code-simplification`, and `code-review-and-quality` (those three are driven by the `simplify` / `code_review` headers per `_lib/review_skills.md`, never by discovery), and any skill gated to a platform or condition that is not currently active.
