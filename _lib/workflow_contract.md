...
6. **Defect-targeted diversification (sequencing exception):** when a `[PARALLEL EXECUTION]` step contains both a **Devils Advocate** row and a **Diversifier** row and both gates resolve `on`, launch the Devils Advocate (and every other listed subagent) in parallel first, wait for the Devils Advocate's result, then launch the Diversifier with that result included in its spawn prompt under the label `[known defects]` — alternatives aimed at confirmed defects are adopted far more often than ones aimed at hypothesized limits, and that is worth the serialized wall-clock. When the Devils Advocate gate resolves `off` (its default), this exception does not apply and the step runs fully parallel as tagged. If the Devils Advocate ends in `fallback-single-agent` / `blocked`, launch the Diversifier without `[known defects]` rather than blocking it.
