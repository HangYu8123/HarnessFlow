...
- **Analysis-subagent gates (`devils_advocate`, `online_research`):** Two binary header toggles decide whether the **Devils Advocate** and **Online Researcher** rows run. `devils_advocate` defaults to **`off`** (the row is skipped unless the header is `on`); `online_research` defaults to **`on`** (the row runs unless the header is `off`). Any absent or unrecognized value resolves to that default. `on` spawns the subagent exactly as its subagent-table row (or numbered step) specifies; `off` skips it entirely and leaves its output label unproduced — downstream steps consume whichever labels were produced, exactly as `_lib/review_skills.md` handles a skipped review skill. The **Diversifier** has no gate — it always runs. **Exception (loop family):** the Devils Advocate that co-guards the exit gate (the *exit-gater*) is a safety guardrail and runs regardless of `devils_advocate`; the toggle governs only the advisory spec-critique pass. (Request templates ship with `devils_advocate: off` and `online_research: on`; `initialize` omits both because that family spawns neither subagent.)
