...
- **Subagent model (specified by the instructions):** Before creating any subagent, resolve the model the instructions specify for subagents — the `subagent_model` header value — and refer to it as [specified subagent model]; also note the model you are yourself running as [main agent model]. Create every subagent with the exact [specified subagent model]. When `subagent_model` is a specific model id, all subagents run on that exact id (a deliberate override — honor it even if it is smaller than [main agent model]). When `subagent_model` is `inherit` or unset, [specified subagent model] falls back to [main agent model] — the model the main agent is running — which must not be downgraded: in **fast mode** (`mode: fast`) the default main model is **Sonnet 4.6**, so `inherit` subagents run on Sonnet 4.6; in **general** and **skill** modes `inherit` subagents run on whatever model the main agent is running. (Request templates ship with `subagent_model: claude-opus-5`.)
- **Subagent effort (specified by the instructions):** Effort is the **second dial on every spawn** — resolve it alongside [specified subagent model], never instead of it. Resolve the `subagent_effort` header (`inherit` | `low` | `medium` | `high` | `xhigh` | `max` — Claude Code's documented effort scale plus `inherit`; available levels depend on the model) as [specified subagent effort]. **`inherit`, or an absent header, means use the session/main-agent effort:** set no platform effort field and add no `effort:` prompt line, and record `effort: inherit` in the activity log. Any other value is a deliberate override and must reach the subagent through one of two channels. **(a) The installed agent definition** — `.claude/agents/*.md` (and `--agents` JSON) supports an `effort:` frontmatter field, `.codex/agents/*.toml` a `model_reasoning_effort` field; both are separate from the model and both override the session effort, inheriting from it when unset. This is the *enforced* channel, but it is **per role, not per request**: to use it, set `effort:` in the source `agents/<slug>.agent.md` frontmatter and re-run `sync_agent_definitions.py` to propagate it (Codex clamps `xhigh`/`max` to `high`). **(b) The prompt** — neither the Claude Code `Task` tool nor a Codex worker exposes a **per-invocation** effort parameter, so a per-request `subagent_effort` header that is not already baked into the definition must be enforced through the prompt: include the line `effort: [specified subagent effort] — binding budget, not a hint` in the subagent prompt, and record `effort: prompt-enforced` in the activity log. `_lib/subagent_contract.md` §Working Rules binds the subagent to that line. Record `effort: not-applied` only when neither channel exists — never block or fail a launch over effort. Role-specific effort headers (see §Online Researcher effort below) override [specified subagent effort] for the named role when present. (Request templates ship with `subagent_effort: low`.)
- **Online Researcher effort (specified by the instructions):** Resolve the `online_researcher_effort` header (same scale, `inherit` included) as [specified online researcher effort]. When spawning the **Online Researcher** subagent (`agents/online-researcher.agent.md`), use [specified online researcher effort] in place of [specified subagent effort] — a deliberate per-role override: honor it even when it is *lower* than [specified subagent effort], exactly as a specific `subagent_model` is honored even when smaller. The same two-channel enforcement applies: set the platform effort field where it exists, otherwise carry the level in the prompt and record `effort: prompt-enforced`. `inherit`, or an absent header, falls back to [specified subagent effort] (which itself falls back to the session effort). The model is unaffected — the Online Researcher still uses [specified subagent model]. (Request templates ship with `online_researcher_effort: medium`; `initialize` omits it because that family spawns no Online Researcher.)
- **Analysis-subagent gates (`diversifier`, `devils_advocate`, `online_research`):** Three binary header toggles decide whether the corresponding **Diversifier**, **Devils Advocate**, and **Online Researcher** rows run when a workflow contains those rows. `diversifier` and `online_research` default to **`on`** (their rows run unless the corresponding header is `off`); `devils_advocate` defaults to **`off`** (the row is skipped unless the header is `on`). Any absent or unrecognized value resolves to that gate's default. `on` spawns the subagent exactly as its subagent-table row (or numbered step) specifies; `off` skips it entirely and leaves its output label unproduced — downstream steps consume whichever labels were produced, exactly as `_lib/review_skills.md` handles a skipped review skill. **Exception (loop family):** the Devils Advocate that co-guards the exit gate (the *exit-gater*) is a safety guardrail and runs regardless of `devils_advocate`; the toggle governs only the advisory spec-critique pass. Request templates ship applicable gates as `diversifier: on`, `devils_advocate: off`, and `online_research: on`; query and loop omit `diversifier`, while initialize omits all three, because those workflows do not contain the corresponding analysis rows.
- **Subagent turn cap (derived from effort):** every effort level implies a turn budget — one turn = one assistant step; a batch of parallel tool calls counts as one turn: `low` 12 · `medium` 20 · `high` 30 · `xhigh` 45 · `max` 60. It reaches the subagent through the same two channels as effort. **(a) The installed agent definition:** every generated definition carries the cap — a native `maxTurns:` frontmatter field where the format has one, a `turn budget:` line in the definition body otherwise — set automatically by `sync_agent_definitions.py` from the def's pinned `effort:` tier when one is set, from an explicit `max_turns:` source-frontmatter override when given, else the `max`-tier backstop (60), since a def with no pinned effort runs at whatever effort the request sets. **(b) The prompt (per request):** whenever the spawn's resolved effort is not `inherit`, append the line `turn budget: at most <N> tool-call turns` (N from the table, same level as the `effort:` line) to the subagent prompt; `_lib/subagent_contract.md` §Working Rules binds the subagent to it. The definition cap stays the outer ceiling — a subagent stopped by it returns whatever it has, so treat such a result as incomplete under the returned-result quality check rather than as a launch failure.
...
- Each subagent prompt must include: the exact task, required inputs, context files to read, the expected output label, and the `effort:` line whenever [specified subagent effort] for that role is not `inherit` (see §Subagent effort). Ad-hoc fallback prompts — spawns with no installed agent definition (see §Subagent Invocation) — must additionally include the role/mode and references to `_lib/subagent_contract.md` and `philosophy/philosophy.instructions.md`.
